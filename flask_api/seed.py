from app import app, db, Story, Page, Choice
...
def seed_story():
    with app.app_context():
        ...
        p3 = Page(story_id=ford_story.id, text="You ignored Ken to save money. At the first corner of Le Mans, your brakes explode. You crash out in front of Henry Ford II. You're fired. GAME OVER.", is_ending=True)
        ...
        p5 = Page(story_id=ford_story.id, text="Ken slows down. The Fords cross together. But because McLaren started further back on the grid, he is declared the winner on a technicality. Ken is robbed of his win, but his legacy is secure. ENDING.", is_ending=True)
        
        p6 = Page(story_id=ford_story.id, text="Ken ignores the order and floors it! He wins Le Mans clearly. Ford is furious you disobeyed corporate, but the world knows who the fastest man is. VICTORY. ENDING.", is_ending=True)
        ...
        db.session.add(Choice(page_id=p2.id, text="Play it safe: Stay at 6,000 RPM", next_page_id=p3.id)) # Ends in failure
        ...
        db.session.commit()
        print("🏁 Full Le Mans scenario loaded into Postgres!")
